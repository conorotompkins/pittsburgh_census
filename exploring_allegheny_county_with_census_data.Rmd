...
This code ceates a dataframe of some of the variables available through the census API.
```{r}
...
This code maps the data onto the census tract geographies. The st_erase function makes the rivers show up on the map.
```{r}
...
This code requests information about the ethnicities within each census tract. Then, it calculates the percentage of the entire population of a tract that each enthnicity makes up.
```{r}
...
allegheny_race %>%
  ...
  facet_wrap(~variable) +
  geom_sf() +
  scale_fill_viridis("Percent", labels = percent) +
  scale_color_viridis("Percent", labels = percent)
```
